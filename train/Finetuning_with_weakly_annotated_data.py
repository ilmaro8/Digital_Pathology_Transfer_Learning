...
MODELS_PATH = #PATH_MODEL_FINETUNING
MODELS_PATH = MODELS_PATH + 'finetuning/'
models_path = MODELS_PATH + 'perc_' + PERCENTAGE_DATA_str + '/'
models_path = MODELS_PATH + '/exp_'+N_EXP+'/'

csv_dir = #PATH_CSV_WEAKLY_ANNOTATED_DATA
	#if 100%
csv_train = csv_dir + 'train_weak.csv'
csv_valid = csv_dir + 'valid_weak.csv'
csv_test = csv_dir + 'test_weak.csv'
...
model_to_finetune = #PATH_BEST_PRETRAINED_MODEL

model_to_finetune = #MODEL_PATH_STRONG
model_to_finetune = model_to_finetune + 'fully_supervision/'
model_to_finetune = model_to_finetune + 'perc_' + PERCENTAGE_DATA_str + '/'
model_to_finetune = model_to_finetune + 'N_EXP_' + N_EXP_str + '/'
model_filename_base = model_to_finetune+ 'best_finetuned_TMA_model.h5'
...
history = model.fit_generator(train_gen, train_steps, shuffle=True, epochs=epochs, verbose=1, validation_data=valid_gen, validation_steps=val_steps,use_multiprocessing = True, workers = 10, max_queue_size=64, callbacks=[checkpoint1,checkpoint2])#, class_weight=class_weight)
