...
model_double = Model(inputs=pre_trained_model.input, outputs=[x1, x2])
opt = Adam(lr=1e-3, beta_1=0.9, beta_2=0.999, decay=1e-3 / 100)
model_double.compile(loss={'output_1': 'categorical_crossentropy', 'output_2': 'categorical_crossentropy'},optimizer=opt, metrics=['acc'])
...
try:
    ...
    try:
        model_double.load_weights(model_path)
        print("WEIGHTS LOADED")
    except:
